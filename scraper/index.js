...
async function predicateItemId(row, slot) {
  ...
  for (const itemId of flattenedPredictions) {
    try {
...
async function main() {
  ...
  // var bisAddonData = JSON.parse(fs.readFileSync(join(appDir, "cached_bisList.json")));

  // let template = fs.readFileSync(join(appDir, "./bisListTemplate.liquid"), "utf8");
  // engine
